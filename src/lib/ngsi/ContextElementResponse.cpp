...
)
{
  ...
  // Entity
  BSONObj id = getField(entityDoc, "_id", __FUNCTION__).embeddedObject();
  contextElement.entityId.fill(getStringField(id, ENT_ENTITY_ID, __FUNCTION__), getStringField(id, ENT_ENTITY_TYPE, __FUNCTION__), "false");
  contextElement.entityId.servicePath = id.hasField(ENT_SERVICE_PATH) ? getStringField(id, ENT_SERVICE_PATH, __FUNCTION__) : "";

  /* Get the location attribute (if it exists) */
  std::string locAttr;
  if (entityDoc.hasElement(ENT_LOCATION))
  {
    locAttr = getStringField(getObjectField(entityDoc, ENT_LOCATION, __FUNCTION__), ENT_LOCATION_ATTRNAME, __FUNCTION__);
  }
  ...
  //
  BSONObj                attrs = getField(entityDoc, ENT_ATTRS, __FUNCTION__).embeddedObject();
  ...
  for (std::set<std::string>::iterator i = attrNames.begin(); i != attrNames.end(); ++i)
  {
    ...
    ca.type           = getStringField(attr, ENT_ATTRS_TYPE, __FUNCTION__);
    ...
    else
    {
      ...
      {
      case String:
        ca.stringValue = getStringField(attr, ENT_ATTRS_VALUE, __FUNCTION__);
      ...
      case NumberDouble:
        ca.numberValue = getField(attr, ENT_ATTRS_VALUE, __FUNCTION__).Number();
        caP = new ContextAttribute(ca.name, ca.type, ca.numberValue);
        break;

      case NumberInt:
        ca.numberValue = (double) getIntField(attr, ENT_ATTRS_VALUE, __FUNCTION__);
        caP = new ContextAttribute(ca.name, ca.type, ca.numberValue);
        break;

      case Bool:
        ca.boolValue = getBoolField(attr, ENT_ATTRS_VALUE, __FUNCTION__);
      ...
      case Object:
        ...
        compoundObjectResponse(caP->compoundValueP, getField(attr, ENT_ATTRS_VALUE, __FUNCTION__));
        break;

      case Array:
        caP = new ContextAttribute(ca.name, ca.type, "");
        caP->compoundValueP = new orion::CompoundValueNode(orion::ValueTypeVector);
        compoundVectorResponse(caP->compoundValueP, getField(attr, ENT_ATTRS_VALUE, __FUNCTION__));
        break;
    ...
    {
      std::vector<BSONElement> metadataV = getField(attr, ENT_ATTRS_MD, __FUNCTION__).Array();
  ...
  {
    ContextAttribute* caP = new ContextAttribute(DATE_CREATED, DATE_TYPE, (double) getIntOrLongFieldAsLong(entityDoc, ENT_CREATION_DATE, __FUNCTION__));
    contextElement.contextAttributeVector.push_back(caP);
  }

  if (includeModDate && entityDoc.hasField(ENT_MODIFICATION_DATE))
  {
    ContextAttribute* caP = new ContextAttribute(DATE_MODIFIED, DATE_TYPE, (double) getIntOrLongFieldAsLong(entityDoc, ENT_MODIFICATION_DATE, __FUNCTION__));
    contextElement.contextAttributeVector.push_back(caP);
  }
}
