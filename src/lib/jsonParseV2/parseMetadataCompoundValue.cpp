...
*/
std::string parseMetadataCompoundValueStandAlone
(
  Document&            document,
  Metadata*            mdP,
...
)
{
  ...
  mdP->compoundValueP->name      = "TOP";
  mdP->compoundValueP->container = mdP->compoundValueP;
  mdP->compoundValueP->valueType = mdP->valueType;  // Convert to other type?
  mdP->compoundValueP->path      = "/";
  mdP->compoundValueP->rootP     = mdP->compoundValueP;
  mdP->compoundValueP->level     = 0;
  mdP->compoundValueP->siblingNo = 0;
  ...
  // Children of the node
  //
  if (mdP->valueType == orion::ValueTypeVector)
  {
    int counter  = 0;
  ...
  else if (mdP->valueType == orion::ValueTypeObject)
  {
    int counter  = 0;
