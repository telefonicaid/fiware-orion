...
static void doNotifyKafka(SenderThreadParams* params)
{
  ...
  if (kafkaMgr.sendKafkaNotification(endpoint.c_str(), params->resource, params->content, params->subscriptionId, params->tenant, params->servicePath, params->extraHeaders, params->user, params->passwd, params->saslMechanism, params->securityProtocol, params->kafkaKeyIsNull, params->kafkaKey, params->fiwareCorrelator, params->renderFormat))
  {
    ...
    // For Kafka, sendKafkaNotification() returning true only means the producer accepted
    // and queued the message. The real notification result is reported asynchronously
    // later by the Kafka delivery callback.
  }
  else
  {
    subNotificationOutcome(params->tenant, params->subscriptionId, true, -1, "", params->failsCounter, params->maxFailsLimit);
