...
)
{
  ...
  {
    LM_T(LmtSubCache, ("No match due to Entity ID"));
    matchedId = false;
  }
  ...
  if (matchedId)
  {
    ...
    {
      LM_T(LmtSubCache, ("No match due to Entity Type"));
      matchedType = false;
    }
    else
    {
...
)
{
  ...
  if (subCacheMultitenant == true)
  {
    if ((cSubP->tenant == NULL) || (tenant == NULL) || (cSubP->tenant[0] == 0) || (tenant[0] == 0))
    {
      if ((cSubP->tenant != NULL) && (cSubP->tenant[0] != 0))
      {
        ...
        LM_T(LmtSubCache, ("No match due to tenant I"));
        return false;
      }
      ...
      if ((tenant != NULL) && (tenant[0] != 0))
      {
        ...
        LM_T(LmtSubCache, ("No match due to tenant II"));
        return false;
      }
    }
    else if (strcmp(cSubP->tenant, tenant) != 0)
    {
      ...
      LM_T(LmtSubCache, ("No match due to tenant III"));
      return false;
    }
  }
  ...
  if (servicePathMatch(cSubP, (char*) servicePath) == false)
  {
    ...
    LM_T(LmtSubCache, ("No match due to servicePath"));
    return false;
  }
  ...
  if (!attributeMatch(cSubP, attrV))
  {
    ...
    LM_T(LmtSubCache, ("No match due to attributes"));
    return false;
  }
...
void debugSubCache(const char* prefix, const char* title)
{
  ...
  LM_T(LmtSubCache, ("%s%s", prefix, title));
  while (subP != NULL)
  {
    LM_T(LmtSubCache, ("%s  * Subscription %s:",       prefix, subP->subscriptionId));
    LM_T(LmtSubCache, ("%s    - lastNotification: %f", prefix, subP->lastNotificationTime));
    LM_T(LmtSubCache, ("%s    - lastSuccess:      %f", prefix, subP->lastSuccess));
    LM_T(LmtSubCache, ("%s    - lastFailure:      %f", prefix, subP->lastFailure));
    LM_T(LmtSubCache, ("%s    - timesSent:        %d", prefix, subP->count));
    LM_T(LmtSubCache, ("%s", prefix));
