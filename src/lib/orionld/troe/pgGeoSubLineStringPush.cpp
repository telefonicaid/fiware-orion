...
)
{
  ...
  if (observedAt != NULL)
  {
    snprintf(sql, sqlSize, "INSERT INTO subAttributes("
             "instanceId, ts, id, entityId, attrInstanceId, attrDatasetId, observedAt, valueType, geoLineString) "
             "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', '%s', 'GeoLineString', ST_GeomFromText('LINESTRING(%s)', 4326))",
             instanceId, orionldState.requestTimeString, subAttributeName, entityId, attrInstanceId, attrDatasetId, observedAt, lineStringCoordsString);
  }
  else
  {
    snprintf(sql, sqlSize, "INSERT INTO subAttributes("
             "instanceId, ts, id, entityId, attrInstanceId, attrDatasetId, valueType, geoLineString) "
             "VALUES ('%s', '%s', '%s', '%s', '%s', '%s', 'GeoLineString', ST_GeomFromText('LINESTRING(%s)', 4326))",
             instanceId, orionldState.requestTimeString, subAttributeName, entityId, attrInstanceId, attrDatasetId, lineStringCoordsString);
  }
