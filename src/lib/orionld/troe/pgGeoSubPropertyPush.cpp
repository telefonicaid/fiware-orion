...
)
{
  ...
  if (strcmp(geoTypeP->value.s, "Point") == 0)
    ok = pgGeoSubPointPush(connectionP, coordinatesP, instanceId, entityId, attrInstanceId, attrDatasetId, subAttributeName, observedAt);
  else if (strcmp(geoTypeP->value.s, "Polygon") == 0)
    ok = pgGeoSubPolygonPush(connectionP, coordinatesP, instanceId, entityId, attrInstanceId, attrDatasetId, subAttributeName, observedAt);
  else if (strcmp(geoTypeP->value.s, "LineString") == 0)
    ok = pgGeoSubLineStringPush(connectionP, coordinatesP, instanceId, entityId, attrInstanceId, attrDatasetId, subAttributeName, observedAt);
  else if (strcmp(geoTypeP->value.s, "MultiPolygon") == 0)
    ok = pgGeoSubMultiPolygonPush(connectionP, coordinatesP, instanceId, entityId, attrInstanceId, attrDatasetId, subAttributeName, observedAt);
  else if (strcmp(geoTypeP->value.s, "MultiLineString") == 0)
    ok = pgGeoSubMultiLineStringPush(connectionP, coordinatesP, instanceId, entityId, attrInstanceId, attrDatasetId, subAttributeName, observedAt);
  else
