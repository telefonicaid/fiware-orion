...
bool legacyPostSubscriptions(void)
{
  ...
  if (endpointP != NULL)
  {
    ...
    LM_T(LmtLegacy, ("1178: uri: '%s'", uriP->value.s));
    if ((strncmp(uriP->value.s, "mqtt://", 7) == 0) || (strncmp(uriP->value.s, "mqtts://", 8) == 0))
    {
      ...
      LM_T(LmtLegacy, ("1178: parsing MQTT url: '%s'", uriP->value.s));
  ...
  if (mqtt)
  {
    ...
    //
    LM_T(LmtLegacy, ("1178: Establishing connection with the MQTT broker"));
