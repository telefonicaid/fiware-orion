...
-   HTTP および Kafka のカスタム・ヘッダ値では、JEXL 式で引用符付き文字列リテラルをサポートするために、`'` と `"` の引用符文字が使用可能になります
...
-   値が `true` の `TextUnrestricted` という名前のブール型メタデータを持つ属性値（[特殊メタデータ型](#special-metadata-types) セクションを参照）
...
- `TextUnrestricted`: 属性に値 `true` を持つ `TextUnrestricted` という名前のブール型メタデータが追加されると、Orion は属性値に対する [構文制限](#general-syntax-restrictions) チェックをスキップします。これにより、属性タイプは他のカスタムタイプに使用できるようになります。例:
...
クライアントは、`notification.httpCustom`, `notification.mqttCustom` または `notification.kafkaCustom` が使用されている場合、単純な
...
- `headers`（ヘッダ名と値の両方をテンプレート化できます）。ただし、カスタム通知において `Fiware-Correlator` および
  `Ngsiv2-AttrsFormat` ヘッダを上書きすることはできません。上書きを試みた場合
  (例: `"kafkaCustom": { ... "headers": {"Fiware-Correlator": "foo"} ...}`)、その設定は無視されます。
- `payload`, `json`, `ngsi` (すべてペイロード関連のフィールド)
- `topic`
- `key`
...
| `lastFailureReason`                                                                                                                                                                                                                                                                                | 取得時のみ | string  | 編集できません。GET 操作にのみ存在します。最後の失敗の原因を説明します (つまり、失敗は `lastFailure` 時に発生しました)。MQTT と KAFKA サブスクリプションには含まれていません                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| `lastSuccessCode`                                                                                                                                                                                                                                                                                  | 取得時のみ | number  | 編集できません。GET 操作にのみ存在します。成功した通知が最後に送信されたときに受信エンドポイントによって返された HTTP コード (200, 400, 404, 500など) (つまり、成功は `lastSuccess `時間に発生しました)。MQTT と KAFKA サブスクリプションには含まれていません                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
g パラメータ         | オプション | タイプ  | 説明                                                                                                                                                                                                                                                           |
|--------------------|------------|---------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `url`              |            | string  | 使用する KAFKA ブローカーのエンドポイントを表します。URL は `kafka://` で始まる必要があり、パスは含まれません (ホストとポートのみが含まれます)                                                                                                                 |
| `topic`            |            | string  | 使用する KAFKA トピックを表す                                                                                                                                                                                                                                  |
| `key`              | ✓          | string  | 使用するKafka メッセージ・キーを指定します。省略した場合、または `null` に設定した場合、通知は null キーで送信されます。それ以外の場合は、指定された文字列値が Kafka メッセージキーとして使用されます。                                                        |
| `user`             | ✓          | string  | Kafka SASL 認証に使用するユーザ名。`passwd` と併用する必要があります                                                                                                                                                                                           |
| `passwd`           | ✓          | string  | Kafka SASL 認証に使用されるパスフレーズ。`user` と組み合わせて使用する必要があります。サブスクリプション情報を取得する際（例：`GET /v2/subscriptions`）には、常に難読化されます                                                                                |
| `saslMechanism`    | ✓          | string  | 認証が有効になっている場合に使用する SASL メカニズム。指定可能な値は、`PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512` です。`user`/`passwd` が設定されている場合は必須です                                                                                           |
g `securityProtocol` | ✓          | string  | 認証が有効になっている場合に使用する Kafka セキュリティ・プロトコル。指定可能な値は `SASL_PLAINTEXT` と `SASL_SSL` です。省略した場合、認証が有効になっている場合 (つまり、`user` と `passwd` が設定されている場合) は、デフォルトで `SASL_SSL` が使用されます |
...
| `key`              | ✓          | string  | 使用するKafka メッセージ・キーを指定します。このフィールドに対してもマクロ置換が実行されます。省略した場合、または `null` に設定した場合、通知は null キーで送信されます。                                                                                     |
| `headers`          | ✓          | object  | 通知メッセージに含まれるヘッダのキーマップ。空にしてはいけません                                                                                                                                                                                               |
...
| `passwd`           | ✓          | string  | Kafka SASL 認証に使用されるパスフレーズ。`user` と組み合わせて使用する必要があります。サブスクリプション情報を取得する際（例：`GET /v2/subscriptions`）には、常に難読化されます                                                                                |
| `saslMechanism`    | ✓          | string  | 認証が有効になっている場合に使用する SASL メカニズム。指定可能な値は、`PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512` です。`user`/`passwd` が設定されている場合は必須です                                                                                           |
| `securityProtocol` | ✓          | string  | 認証が有効になっている場合に使用する Kafka セキュリティ・プロトコル。指定可能な値は `SASL_PLAINTEXT` と `SASL_SSL` です。省略した場合、認証が有効になっている場合 (つまり、`user` と `passwd` が設定されている場合) は、デフォルトで `SASL_SSL` が使用されます |
