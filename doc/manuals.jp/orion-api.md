...
| `forcedUpdate`     | 更新操作では、属性が効果的に更新された場合にのみ更新されるデフォルトの動作の代わりに、実際の属性の更新があるかどうかに関係なく、一致するサブスクリプションをトリガーする必要があります。同じ効果については、 `entityChange` の[変更タイプ](user/subsets_alttype.md)も確認してください |
...
詳細については、[問題診断手順書](admin/Diagnostics.md#diagnose-notification-reception-problems) のセクションを
参照してください。
