...
| `expression` | ✓          | object | `q`, `mq`, `georel`, `geometry` および `coords` で構成される式 (このフィールドについては、上記の "エンティティの一覧表示" の操作を参照してください) |
...
| `metadata`                   | ✓          | string  | 通知メッセージに含まれるメタデータのリスト。詳細については、"属性とメタデータの除外" のセクションを参照してください                                                                                                                                                        |
