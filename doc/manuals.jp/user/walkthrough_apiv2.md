...
[このウォークスルーの NGSIv1 version](walkthrough_apiv1.md) もあります。NGSIv2 でまだ開発されていないコンテキスト管理のアベイラビリティ機能 (別名 NGSI9) が必要な場合を除いて、一般的に、NGSIv2、つまりこのドキュメントを使用する必要があります。疑わしい場合は、NGSIv2 を使用する必要があります。
...
メイン・セクションは [コンテキスト管理](#context-management)です。コンテキスト管理 (車の温度などのエンティティに関する情報) の基本的なコンテキスト broker 機能について説明します。これを使用するために考慮するいくつかの意見です。

NGSIv2 の現在の候補バージョンでは、コンテキストのアベイラビリティ管理 (エンティティ自身に関する情報ではなく、その情報の提供者に関する情報) はまだ開発されていません。これについてのセクションはこのドキュメントの末尾にありますが、将来この情報を含めるプレースホルダとして存在します。この機能を使用したければ、NGSIv1 を使用する必要があります。詳細については、 [NGSIv1 のウォークスルー](walkthrough_apiv1.md)を参照してください。
