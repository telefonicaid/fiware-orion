...
* `user` (オプション) Kafka SASL 認証に使用するユーザー名。`passwd` と併用する必要があります
* `passwd` (オプション) Kafka SASL 認証に使用するパスワード。`user` と併用する必要があります
* `saslMechanism` (`user`/`passwd` が設定されている場合は必須) 使用する SASL メカニズム。指定可能な値は以下のとおりです:
  `PLAIN`, `SCRAM-SHA-256`, `SCRAM-SHA-512`。
* `securityProtocol` (オプション) SASL 認証が有効になっている場合に使用するKafkaセキュリティプロトコル。
  指定可能な値は以下のとおりです: `SASL_PLAINTEXT`、`SASL_SSL`。

## Kafka 通知におけるメッセージの構造

* `Key`: Kafka がパーティショニングに使用するメッセージ・フィールド。Orion では、このフィールドには null 値が送信されるため、プロデューサの設定に基づいて宛先パーティションが自動的に割り当てられます。
