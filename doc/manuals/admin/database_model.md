...
-   **mdFlags**: a boolean field that specifies if metadata flags has to be included
    in notifications (if `mdFlags` is equal to `true`) or not (if `mdFlags` is equal
    to `false` or doesn't exist).
