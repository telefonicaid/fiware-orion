...
TEST(mongoUpdateContextSubscription, matchEntN_AttrN_C1)
{
    ...
    EXPECT_CALL(*notifierMock, sendNotifyContextRequest(MatchNcr(&expectedNcr),"http://notify4.me", "", "", "no correlator", NGSI_V1_LEGACY, attrsFilter, false))
            .Times(1);
    setNotifier(notifierMock);
...
TEST(mongoUpdateContextSubscription, MongoDbUpdateFail)
{
    ...
    EXPECT_EQ("Internal Server Error", res.subscribeError.errorCode.reasonPhrase);
    EXPECT_EQ("Database Error (collection: utest.csubs "
              "- update(): <{ _id: ObjectId('51307b66f481db11bf860001') },{ expiration: 1360250700, reference: \"http://notify1.me\", extended: false, servicePath: \"/#\", entities: [ { id: \"E1\", type: \"T1\", isPattern: \"false\" } ], attrs: [], blacklist: false, conditions: [], lastNotification: 15000000, expression: {}, format: \"JSON\" }> "
              "- exception: boom!!)", res.subscribeError.errorCode.details);

      //Actual: "Database Error (collection: utest.csubs - update(): <{ _id: ObjectId('51307b66f481db11bf860001') },{ expiration: 1360250700, reference: "http://notify1.me", extended: false, servicePath: "/#", entities: [ { id: "E1", type: "T1", isPattern: "false" } ], attrs: [], blacklist: false, conditions: [], lastNotification: 15000000, expression: {}, format: "JSON" }> - exception: boom!!)"
