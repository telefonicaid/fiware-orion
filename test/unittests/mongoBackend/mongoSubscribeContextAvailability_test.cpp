...
TEST(mongoSubscribeContextAvailability, noPatternAttrsAll)
{
    ...
    NotifierMock* notifierMock = new NotifierMock();
    EXPECT_CALL(*notifierMock, sendNotifyContextAvailabilityRequest(MatchNcar(&expectedNcar),"http://notify.me"))
            .Times(1);
    setNotifier(notifierMock);
...
TEST(mongoSubscribeContextAvailability, MongoDbInsertFail)
{
    ...
    EXPECT_EQ("'), expiration: 1360236300, reference: \"http://notify.me\", entities: [ { id: \"E5\", type: \"T5\", isPattern: \"false\" } ], attrs: {} } "
              "- exception: boom!!", s2);
