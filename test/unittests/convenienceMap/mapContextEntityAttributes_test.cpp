...
/* ****************************************************************************
*
* mapContextEntityAttributes - 
*/
TEST(mapContextEntityAttributes, ok)
{
  HttpStatusCode                       ms;
  ...
  ms = mapPostContextEntityAttributes("Entity01", &rpr, &rcr);
  EXPECT_EQ(SccOk, ms);
  EXPECT_STREQ("PT1S", rcr.duration.string.c_str());

  ms = mapGetContextEntityAttributes("Entity01", &dcar);
  EXPECT_EQ(SccOk, ms);
