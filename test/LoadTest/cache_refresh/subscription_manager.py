...
class Subscriptions:
    ...
    def __usage():
        ...
        print " *     <host>               : CB o balancer host (Mandatory).                                                    *"
        print " *     -operation=<value>   : operation allowed: create | update | delete (by default, create) (Optional).       *"
        print " *     -duration=<value>    : used only in create and update operations (by default, 1000S) (Optional)           *"
        print " *     -throttling=<value>  : used only in create and update operations (by default, it is not used) (Optional)  *"
        print " *     -subs_id=<value>     : used only in delete and update operation (by default, NotDefined) (Optional)       *"
        print " *     -service=<value>     : service header (by default, entities_append_long_time) (Optional)                  *"
        print " *     -service_path=<value>: service path header (by default, /test) (Optional)                                 *"
