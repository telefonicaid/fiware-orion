...
Feature: ServicePath for registrations (NGSI9) (not recursive)
  ...
  Scenario: Subscribe different entities with the same provider and services but different subservice
    # First registration
    Given a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice1"
    ...
    # Second registration
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice2"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
      | Room3     | Room        |
    ...
    And a standard context registration is asked with the before information
    # Discover
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice1"
    And the following entities to consult
  ...
  Scenario: Query context to a subservice
    ...
    # First registration
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice1/1"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And the following attributes to consult
    ...
    And a standard context registration is asked with the before information
    # Second Registration
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice1/2"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And the following attributes to consult
    ...
    And a standard context registration is asked with the before information
    # Query consult
    And a new NGSI "10" api petition with the service "issue_719" and the subservice "/subservice1/2"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
  ...
  Scenario: Update an entity in a context provider, having other context provider with the same entity in other subservice
    ...
    And a standard context registration is asked with the before information
    # Update operation
    And a new NGSI "10" api petition with the service "issue_719" and the subservice "/subservice1"
  ...
  Scenario: Entity in CB and in CP with the same subervice, and in other CP with other subservice.
    ...
    And a standard context entity creation is asked with the before information
    # Query operation
    And a new NGSI "10" api petition with the service "issue_719" and the subservice "/subservice2"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And build the standard query context payload with the previous data
  ...
  Scenario: Entity in CB and in CP without servicepath, and in other CP with other servicepath.
    ...
    And set the response of the mock in the path "/context_provider/service2/queryContext" as "query_context_response_from_context_provider_xml"
    # First registration
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "empty"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And the following attributes to consult
    ...
    And a standard context registration is asked with the before information
    # Second Registration
    And a new NGSI "9" api petition with the service "issue_719" and the subservice "/subservice"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And the following attributes to consult
    ...
    And a standard context registration is asked with the before information
    # Append operation
    And a new NGSI "10" api petition with the service "issue_719" and the subservice "empty"
    And the following attributes to create
      | attribute_name | attribute_type | attribute_value |
      | att1           | att_type_1     | 25              |
    And a context elements with the before attrs and the following entities
    ...
    And a standard context entity creation is asked with the before information
    # Query operation
    And a new NGSI "10" api petition with the service "issue_719" and the subservice "/subservice"
    And the following entities to consult
      | entity_id | entity_type |
      | Room1     | Room        |
    And build the standard query context payload with the previous data
