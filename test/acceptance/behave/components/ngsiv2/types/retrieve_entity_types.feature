...
# ------------------ queries parameters -------------------------------
  ...
  @only_limit_zero
  Scenario Outline:  try to get entities type using NGSI v2 with only limit query parameter with zero value
    Given  a definition of headers
      | parameter          | value                      |
      | Fiware-Service     | test_type_only_limit_error |
      | Fiware-ServicePath | /test                      |
    ...
    And verify an error response
      | parameter   | value                                                             |
      | error       | BadRequest                                                        |
      | description | Bad pagination limit: /<limit>/ [a value of ZERO is unacceptable] |
    Examples:
      | limit |
      |       |
      | 0     |
