...
echo CURL: curl -3 $url --header "$IN_FORMAT" --header "$OUT_FORMAT" --header "x-fiware-tenant: $httpTenant" $CURL_VERBOSE -X $method $cert
# -----------------------------------------------------------------------------
#
# curl
#
...
if [ "$useXmlLint" == 1 ] 
then
  (curl -3 $url --header "$IN_FORMAT" --header "$OUT_FORMAT" --header "x-fiware-tenant: $httpTenant" $CURL_VERBOSE -X $method $cert -d @- | xmllint --format -) << EOF
$(echo $data)
EOF
else
  (curl -3 $url --header "$IN_FORMAT" --header "$OUT_FORMAT" --header "x-fiware-tenant: $httpTenant" $CURL_VERBOSE -X $method $cert -d @-) << EOF
$(echo $data)
EOF
fi
