...
function startMintakaDB()
{
  ...
  export sql1="create database test_db11;"
  export sql2="\c test_db11"
  export sql3="CREATE EXTENSION IF NOT EXISTS postgis;"
  export sql4="CREATE TABLE IF NOT EXISTS entity_table (entity_id TEXT NOT NULL,entity_type TEXT, geo_property GEOMETRY,created_at TIMESTAMP,modified_at TIMESTAMP, observed_at TIMESTAMP,PRIMARY KEY (entity_id));"
  export sql5="CREATE EXTENSION IF NOT EXISTS timescaledb;"
  export sql6="create type attribute_value_type_enum as enum ('value_string', 'value_number', 'value_boolean', 'value_relation', 'value_object', 'value_datetime', 'value_geo');"
  export sql7="CREATE TABLE IF NOT EXISTS attributes_table (entity_id TEXT NOT NULL REFERENCES entity_table(entity_id),id TEXT NOT NULL, name TEXT, value_type attribute_value_type_enum, sub_property BOOL, unit_code TEXT, data_set_id TEXT, instance_id bigint GENERATED BY DEFAULT AS IDENTITY(START WITH 1 INCREMENT BY 1), value_string TEXT, value_boolean BOOL, value_number float8, value_relation TEXT, value_object TEXT, value_datetime TIMESTAMP,geo_property GEOMETRY,created_at TIMESTAMP NOT NULL,modified_at TIMESTAMP NOT NULL, observed_at TIMESTAMP NOT NULL,PRIMARY KEY (entity_id, id,observed_at,created_at,modified_at));"
  export sql8="SELECT create_hypertable('attributes_table', 'modified_at');"
  export sql9="CREATE TABLE IF NOT EXISTS attribute_sub_properties_table (entity_id TEXT NOT NULL,attribute_id TEXT NOT NULL,attribute_instance_id bigint, id TEXT NOT NULL, value_type attribute_value_type_enum,value_string TEXT, value_boolean BOOL, value_number float8, value_relation TEXT,name TEXT,geo_property GEOMETRY,unit_code TEXT, value_object TEXT, value_datetime TIMESTAMP,instance_id bigint GENERATED BY DEFAULT AS IDENTITY(START WITH 1 INCREMENT BY 1),PRIMARY KEY (instance_id));"
  export sql10="drop database test_db11;"
...
function mintakaUrl()
{
  ...
  while [ "$#" != 0 ]
  do
    if [ "$1" == "--header" ]; then          _headers=${_headers}" --header \"$2\""; shift;
    elif [ "$1" == "-H" ]; then                _headers=${_headers}" --header \"$2\""; shift;
    fi
    shift
  done
...
export -f startMintakaDB
export -f mintakaUrl
export -f stopMintakaDB
export -f sqlMintaka
